...
def draw(win, paddles, ball, left_score, right_score):
    ...
    rigth_score_text = SCORE_FONT.render(f'{right_score}',1 , WHITE)
    win.blit(left_score_text, (WIDTH // 4 - left_score_text.get_width()//2, 20))
    win.blit(rigth_score_text, (WIDTH * (3/4) - rigth_score_text.get_width()//2, 20))
...
def main():
    ...
    while run:
        ...
        if left_score >= WINNING_SCORE:
            won = True
            win_text = 'Left Rlayer Won!'
        if right_score >= WINNING_SCORE:
            won = True
            win_text = 'Right Rlayer Won!'
